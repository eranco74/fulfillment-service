...
var _ ffv1.SubnetsServer = (*SubnetsServer)(nil)

type SubnetsServer struct {
	ffv1.UnimplementedSubnetsServer

	logger    *slog.Logger
	delegate  privatev1.SubnetsServer
	inMapper  *GenericMapper[*ffv1.Subnet, *privatev1.Subnet]
	outMapper *GenericMapper[*privatev1.Subnet, *ffv1.Subnet]
}
...
func (b *SubnetsServerBuilder) Build() (result *SubnetsServer, err error) {
	...
	// Create the mappers:
	inMapper, err := NewGenericMapper[*ffv1.Subnet, *privatev1.Subnet]().
		SetLogger(b.logger).
		SetStrict(true).
		Build()
	if err != nil {
	...
	outMapper, err := NewGenericMapper[*privatev1.Subnet, *ffv1.Subnet]().
		SetLogger(b.logger).
		SetStrict(false).
		Build()
	if err != nil {
...
func (s *SubnetsServer) List(ctx context.Context,
	request *ffv1.SubnetsListRequest) (response *ffv1.SubnetsListResponse, err error) {
	...
	publicItems := make([]*ffv1.Subnet, len(privateItems))
	for i, privateItem := range privateItems {
		publicItem := &ffv1.Subnet{}
	...
	response = &ffv1.SubnetsListResponse{}
...
func (s *SubnetsServer) Get(ctx context.Context,
	request *ffv1.SubnetsGetRequest) (response *ffv1.SubnetsGetResponse, err error) {
	...
	// Create the public response:
	response = &ffv1.SubnetsGetResponse{}
	response.SetObject(publicSubnet)
	return
}

func (s *SubnetsServer) Create(ctx context.Context,
	request *ffv1.SubnetsCreateRequest) (response *ffv1.SubnetsCreateResponse, err error) {
	...
	createdPublicSubnet := &ffv1.Subnet{}
	...
	// Create the public response:
	response = &ffv1.SubnetsCreateResponse{}
	response.SetObject(createdPublicSubnet)
	return
}

func (s *SubnetsServer) Update(ctx context.Context,
	request *ffv1.SubnetsUpdateRequest) (response *ffv1.SubnetsUpdateResponse, err error) {
	...
	updatedPublicSubnet := &ffv1.Subnet{}
	...
	// Create the public response:
	response = &ffv1.SubnetsUpdateResponse{}
	response.SetObject(updatedPublicSubnet)
	return
}

func (s *SubnetsServer) Delete(ctx context.Context,
	request *ffv1.SubnetsDeleteRequest) (response *ffv1.SubnetsDeleteResponse, err error) {
	// Create private request:
	...
	// Create the public response:
	response = &ffv1.SubnetsDeleteResponse{}
	return
}
